...
function ProjectCard({ title, description }) {
  return (
    <motion.div
      className="bg-gray-900 border border-gray-700 rounded-xl p-6 shadow-lg hover:shadow-2xl transition duration-300 h-full"
...
function ProjectCarousel({ className = "" }) {
  ...
  return (
    <motion.div
      className={`w-full bg-[#1F1F1F] text-white flex flex-col items-center px-6 py-12 pb-24 m-0 ${className}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="relative w-full max-w-4xl">
        ...
        >
          ...
          {/* Fixed position: outside the carousel item space */}
